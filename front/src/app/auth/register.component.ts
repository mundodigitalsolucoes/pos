    ...
    <div class="auth-page">
      <div class="auth-card" [class.auth-card--wide]="step() > 0">
        ...
        @if (step() === 3) {
          ...
          <div class="photo-uploads">@for (product of onboardedProducts(); track product.id) { <div class="photo-upload-row"><div class="photo-upload-meta"><strong>{{ starterLabel(product.name) }}</strong><span class="starter-currency">R$</span><input type="number" class="starter-product-price" min="0" step="0.01" [value]="product.price_cents / 100" (input)="updateOnboardedPrice(product.id, $event)"></div><input type="file" accept="image/*" (change)="onPhotoSelected(product.id, $event)"></div> }</div>
          @if (error()) { <div class="error-banner">{{ error() }}</div> }
...
export class RegisterComponent implements OnInit {
  ...
  updateStarterPrice(name: string, event: Event): void { const raw=(event.target as HTMLInputElement).value; const reais=parseFloat(raw); const cents=Number.isFinite(reais)?Math.round(reais*100):0; this.starterProducts.update((items)=>items.map((item)=>item.name===name?{...item,priceCents:cents}:item)); }
  updateOnboardedPrice(productId: number, event: Event): void { const raw=(event.target as HTMLInputElement).value; const reais=parseFloat(raw); const cents=Number.isFinite(reais)?Math.round(reais*100):0; this.onboardedProducts.update((items)=>items.map((item)=>item.id===productId?{...item,price_cents:cents}:item)); }
